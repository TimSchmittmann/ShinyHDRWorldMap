# TODO: Almost identical copy from linechart.R. Should be merged
library(shiny)

# To be called from ui.R
barChartOutput <- function(inputId, yaxis=FALSE, width="100%", height="20vh") {
  ...
  tagList(
    ...
    div(id=inputId, class="nvd3-barchart", style=style,
        HTML(paste0("<input id='yaxis' style='display: none' value='",yaxis,"' />")),
        tag("svg", list()))
  )
}
  ...
  function() {
    ...
    mapply(function(col, name) {
      ...
      values <- mapply(function(val, i) {
        list(x = i, y = val)
      }, col, row.names(dataframe), SIMPLIFY=FALSE, USE.NAMES=FALSE)
